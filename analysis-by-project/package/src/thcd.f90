program main
    ...
    call calc_thcd(integ_ej_mean, thcd, thcd_se, fst_calc, lst_calc)
    call output_integ_ej_mean(x, integ_ej_mean, integ_ej_se, ndata)
    call output_thcd(thcd, thcd_se)
contains
    ...
    subroutine calc_thcd(integ_ej_mean, thcd, thcd_se, fst_calc, lst_calc)
        real(real64),intent(out):: thcd, thcd_se
        real(real64),intent(in):: integ_ej_mean(:)
        integer(int32):: fst_calc, lst_calc
        
        thcd = mean(integ_ej_mean(fst_calc:lst_calc), se=thcd_se)
    end subroutine
    ...
    subroutine output_integ_ej_mean(x, integ_ej_mean, integ_ej_se, ndata)
        ...
        open(newunit=u_thcd_mean, file=file_thcd_mean, status='replace')
            write(u_thcd_mean, '(3e20.8)') (x(i), integ_ej_mean(i), integ_ej_se(i), i=1,ndata)
        close(u_thcd_mean)
    end subroutine


    subroutine output_thcd(thcd, thcd_se)
