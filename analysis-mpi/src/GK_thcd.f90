...
contains
    subroutine main_routine()
        ...
        integer(int32):: ndata
        real(real64):: temp, dt, vol, thcd_size
        real(real64),allocatable:: ej(:,:), acf(:), integ(:), x(:)
        ...
        ! input読み込み
        call load_condition_for_GK_thcd_ana(ndata, dt, vol)
        call init_acf(ndata)
        ...
        do i=fst_run+rank, lst_run, procs
            print*, 'reading ... ', '../calculation/'//rundir(i)//'/tdc.dat', rank

            call read_temp(temp, i)
            call read_tdc('../calculation/'//rundir(i)//'/tdc.dat', ndata, ej)
            call calc_acf_ej(ej, ndata, vol, temp, acf)
        ...
        if (rank==0) then
            ! 出力
            thcd_size = dble(all_run)
            do i=1,ndata
                integ_mean(i) = integ_sum_global(i) / thcd_size
                integ_var = integ2_sum_global(i) / (thcd_size-1) - integ_sum_global(i)**2 / thcd_size / (thcd_size-1)
                integ_stdev(i) = sqrt(integ_var)
            end do

            allocate(x(ndata), source=[(dt*dble(i-1), i=1,ndata)])
            call write_arx_ary_arysd('GK_thcd/integ.txt', ndata, x, integ_mean, integ_stdev)
        end if
    end subroutine
    ...
    subroutine calc_acf_ej(ej, ndata, vol, temp, acf)
        ...
        acf(:) = 0d0
        do i=1,3
            acf(:) = acf(:) + auto_correlation_function(ej(:,i))
        end do
        acf(:) = acf(:) / (dble(ndata) * vol*kbt*temp*temp * 3d0)
    end subroutine
end program main