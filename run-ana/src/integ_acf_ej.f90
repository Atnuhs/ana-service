program main
    ...
    call read_mean_temp(temp)
    allocate(ej(ndata, 3))
    allocate(acf_ej(ndata, 3))
    allocate(integ_acf_ej(ndata, 3))

    call read_tcd(ej=ej, ndata=ndata)
    call calc_acf_ej(acf_ej=acf_ej, ej=ej, ndata=ndata)
    ...
    call output_integ_acf_ej(integ_acf_ej=acf_ej, ndata=ndata, dt=dt)
contains
    ...
    subroutine read_mean_temp(temp)
        character(100),parameter:: file_mean_temp='temp/mean_temp.dat'
        real(real64):: temp
        integer(int32):: u_mean_temp
        
        open(newunit=u_mean_temp, file=file_mean_temp, status='old')
            read(u_mean_temp, *) temp
        close(u_mean_temp)
    end subroutine


    subroutine read_tcd(ej, ndata)
        character(100),parameter:: file_tcd='../tcd.dat'
        real(real64),intent(out):: ej(:,:)
        integer(int32),intent(in):: ndata
        integer(int32):: i,u_tcd

        open(newunit=u_tcd,file=file_tcd,status='old')
            read(u_tcd,*) (ej(i,:), i=1,ndata)
        close(u_tcd)
    end subroutine


    subroutine calc_acf_ej(ej, acf_ej, ndata)
        real(real64),intent(out):: acf_ej(:,:)
        integer(int32),intent(in):: ndata
        real(real64),intent(in):: ej(:,:)
        integer(int32):: i
        
        do i=1,3
            acf_ej(:,i) = auto_correlation_function(ej(:,i))
        end do
        acf_ej(:,:) = acf_ej(:,:)/dble(ndata)
    end subroutine
    ...
    subroutine output_acf_ej(acf_ej, ndata, dt)
        ...
        open(newunit=u_acf_ej, file=file_acf_ej, status='replace')
            write(u_acf_ej,'(2e20.10)') (dt*dble(i-1), sum(acf_ej(i,:)), i=1,ndata)
        close(u_acf_ej)
    end subroutine
    ...
    subroutine output_integ_acf_ej(integ_acf_ej, ndata, dt)
        ...
        open(newunit=u_integ_acf_ej, file=file_integ_acf_ej, status='replace')
            write(u_integ_acf_ej,'(2e20.10)') (dt*dble(i-1), sum(integ_acf_ej(i,:)), i=1,ndata)
